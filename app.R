...
#NOT USED FOR GGPLOT IMPLEMENTATION
      ...
      tabItems(
        ...
        tabItem(
          ...
          fluidRow(
            ...
            #I include everything in a column though because this way there are margins and it looks better
            ...
            column(
              ...
              conditionalPanel(
                ...
                tabsetPanel(
                  ...
                  ###### 3rd Plot "chisqCurve" (remarksProb)----
                  tabPanel(
                    h5(
...
server <- function(input, output, session) {
  ...
  observeEvent(
    eventExpr = input$plotClick,
    handlerExpr = {
      v$click1 <- input$plotClick
    }
  )
  ...
  ###### 3rd Plot Output ----
  ChisqDensityArea <- function(bound, region = "above", df = NA) {
    ...
    if (!(region %in% c("below", "above"))) stop("Specify either \"region=\"below\" or \"region=\"above\"")
    if (df < 0) stop("Degrees of freedom must be positive")
    
    if (region == "below") {
      densityFunction <- function(x) dchisq(x, df = df)
    ...
    # Return the density function and area
    return(list(densityFunction = densityFunction, area = area))
  }
  ...
  output$chisqCurve <- renderPlot(
    expr = {
      ...
      densityFunction <- chisqInfo$densityFunction
      area <- chisqInfo$area
      
      # Simulate chi-square 
      simChisq <- rchisq(sim, df = degFreedom)
      
      # plot
      ggplot(data.frame(x = simChisq), aes(x)) +
        theme_bw() +
        geom_area(
          data = data.frame(
            x = seq(
              obs,
              max(obs, qchisq(0.999, df = degFreedom) + 1),
              length.out = 301)
          ),
          aes(x, y = densityFunction(x)
          ),
        ...
        stat_function(
          fun = densityFunction,
          xlim = c(0, max(obs, qchisq(0.999, df = degFreedom) + 1))
        ) +
        geom_line(
          data = as.data.frame(chisqDensities()$y),
          aes(x = chisqDensities()$x, y = chisqDensities()$y),
